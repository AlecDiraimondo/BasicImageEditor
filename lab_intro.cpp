...
PNG warmimage(PNG image)
{
  ...
  for(unsigned x = 0; x < image.width(); x++) {
    for(unsigned y = 0; y < image.height(); y++) {
      ...
      if (pixel.h < yellow && pixel.h > redhue)
      {
        if (yellow - pixel.h < pixel.h - redhue)
        {
          if (yellow - pixel.h < pixel.h - orange)
          {
            pixel.h = yellow;
          }
          else
        ...
        else 
        {
          ...
          else if (pixel.h - redhue < orange - pixel.h)
          {
            pixel.h = redhue;
          }
          else
      ...
      else if (pixel.h < 360 && pixel.h > yellow)
      {
          int dist_yellow = pixel.h - yellow;
          int dist_red = 360 - pixel.h + redhue;

          if (dist_yellow < dist_red)
          {
            pixel.h = yellow;
          }
          else 
...
PNG reverse(PNG image)
{

  unsigned halfwidth = (image.width())/2;
  unsigned halfheight = (image.height())/2;
  //need these values to properly flip the whole image
  ...
  for (unsigned x = 0; x < image.width(); x++){
    for (unsigned y = 0; y < image.height(); y++){
      ...
      HSLAPixel & pixel = image.getPixel(x, y);
      HSLAPixel & newpixel = image.getPixel(image.width()-x-1, image.height()-y-1);
      HSLAPixel & placeholder = pixel;
      ...
      newpixel.h = placeholder.h;
      newpixel.s = placeholder.s;
      newpixel.a = placeholder.a;
      newpixel.l = placeholder.l;

      std::cout << "Done with first half";
  ...
  for (unsigned x = 0; x < halfwidth; x++){
    for (unsigned y = 0; y < halfheight; y++){
      ...
      HSLAPixel & pixel = image.getPixel(halfwidth + x -1, halfheight + y -1);
      HSLAPixel & oppixel = image.getPixel(image.width()-x-1, image.height()-y-1);
